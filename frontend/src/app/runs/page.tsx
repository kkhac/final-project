...
export default function RunsPage() {
  ...
  if (error)
    return <div className="p-8 text-red-500">Failed to load runs.</div>;
    ...
    <div className="p-8 max-w-6xl mx-auto">
      ...
      {runs && runs.length === 0 ? (
        <p className="text-gray-500">
          No runs yet. Trigger one from the Evaluations page.
        </p>
      ) : (
            ...
            <tbody className="divide-y divide-gray-100 bg-white">
              {runs?.map((run) => (
                <tr key={run.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${STATUS_STYLES[run.status]}`}
                    >
                      {run.status}
                    </span>
                  </td>
